...
WebChannel=(()=>{const t="application/json",n={Accept:t,"Content-Type":t};var e={init:function(t){return function(t){Generator.on("call",async(e,r)=>{try{r(null,await async function(t,e){let r=Array.isArray(e.data)&&e.data.length>0,a=e;if(Security.isActive()&&r&&(a=await Security.encrypt(JSON.stringify(e))),a=await async function(t,e){let r=JSON.stringify(e),a={method:"post",heaedrs:n,body:r},c=await fetch(t,a);return await c.json()}(t,a),"err"==a.cmd)throw new Error(a.result.msg);return"enc"===a.cmd&&(a=await Security.decrypt(a)),a}(t,e))}catch(t){r(t,null)}})}(t)}};return Object.freeze(e),e})();
...
Engine=(()=>{async function e(e){e.signature&&(Security.isActive()||await Security.init(e)),Generator.build(e.api)}var n={init:function(n){return async function(n){if(!(n=n||{}).api)throw new Error("API Url not defined!");if(Generator.off("call"),SocketChannel&&SocketChannel.kill(),n.api===n.service&&0==n.api.indexOf("ws"))return async function(n){var i=Date.now();Generator.on("api",async n=>{Generator.off("api"),n.challenge=i,await e(n)}),await SocketChannel.init(n.service+"?q="+i)}(n);if(async function(n){let i=await async function(e){let n=location.pathname.split("/")[1],i=e||location.origin+`/${n}/api`,t=Date.now(),a=await fetch(i,{method:"get",headers:{"x-time":t}}),r=await a.json();return r.challenge=t.toString(),r}(n.api);await e(i)}(n),async function(e){return!!e.service&&(0===e.service.indexOf("http")?(await WebChannel.init(e.service),!0):0===e.service.indexOf("ws")&&(await SocketChannel.init(e.service),!0))}(n))return!0;throw new Error("Invalid definition for Engine Remote Service")}(n)}};return Emitter(n),Object.freeze(n),n})();