...
Security = (() => {
    ...
    async function generateAesKey() {
        let type = { name: "AES-CTR", length: 128 };
    ...
    async function verify(key, signature, challenge) {
        ...
        let type = { name: "ECDSA", hash: { name: "SHA-384" } };
    ...
    async function encryptAesMessage(key, iv, data) {
        ...
        let type = { name: "AES-CTR", counter: iv, length: 128 };
    ...
    async function decryptAesMessage(key, iv, data) {
        ...
        let type = { name: "AES-CTR", counter: counter, length: 128 };
    ...
    async function init(cfg) {
        ...
        VERSION++;
        encKEY = await importRsaKey(cfg.keyEnc, { name: 'RSA-OAEP', hash: 'SHA-256' }, 'encrypt');
        aesKEY = await generateAesKey();
        exportedAES = await exportAesKey(aesKEY);

        let verKey = await importRsaKey(cfg.keyVer, { name: 'ECDSA', namedCurve: "P-384" }, 'verify');        
